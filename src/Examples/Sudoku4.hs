...
module Examples.Sudoku4 where

import Control.Monad.ST (ST)
import Data.Foldable (toList, traverse_)
import Data.List (intercalate)
import Data.Propagator.Cell ( content, cell, write, lift1, Cell )
import Data.Propagator.Class
    ( Propagated(..), Change(Change, Contradiction) )
import Data.Set (Set, (\\))
...
instance Propagated Square where
  merge :: Square -> Square -> Change Square
